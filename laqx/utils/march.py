...
def get_march_update_fn(network, local_energy, args, batch_mcmc_step, num_devices):
    ...
    def learning_rate_schedule(t_):
        lr = jnp.sqrt(args.norm)
        return jnp.minimum(1.0 / (1 + jnp.maximum(t_ - args.lr_start, 0) / args.lr0), t_ / args.warmup) * lr
    
    def cal_update(params, opt_state, data, e_l, t):
        param_flat, unravel_fn = jax.flatten_util.ravel_pytree(params)
        prev_grad, prev_v = opt_state
        ...
        prev_grad_decayed, prev_v, Ohat, diff = prev_grad_decayed.astype(jnp.float64), prev_v.astype(jnp.float64), Ohat.astype(jnp.float64), diff.astype(jnp.float64)
        ...
        Ohat_scale = Ohat_all / (jnp.sqrt(prev_v_pad) + args.eps)

        T = jax.lax.psum(Ohat_scale @ Ohat_all.T, axis_name='batch')
        B = T.shape[0]
        if args.dtype != 'complex':
            ones = jnp.ones((B, 1))
        ...
        dtheta_residual = Ohat_scale.T @ jax.scipy.linalg.solve(T_reg, epsilon_tilde, assume_a="pos")
        ...
        v = args.beta2 * prev_v + (dtheta_residual ** 2)

        norm = jnp.linalg.norm(grad)
        grad_after_norm = grad * learning_rate_schedule(t) / norm

        if args.precision != 'x64':
            grad_after_norm = grad_after_norm.astype(jnp.float32)
        return unravel_fn(param_flat + grad_after_norm), (grad, v), (norm,)

    def march_update_fn(params, opt_state, data, t, subkeys):
        ...
        params, opt_state, aux = cal_update(params, opt_state, data, e_l, t)
