...
def init_nnb(key, args):
    ...
    orbitals = args.particles * spin_channels * args.num_states
    ...
    key, subkey = jax.random.split(key)
    params['output'] = network_block.init_linear_layer(subkey, args.MLP_hidden, orbitals)

    if args.dtype == 'complex':
        params['output_i'] = network_block.init_linear_layer(key, args.MLP_hidden, orbitals)
    return params
...
def apply_nnb(params, pos, cache, args):
    ...
    if args.dtype == 'complex':
        orbitals = orbitals + 1j * network_block.linear_layer(hidden, **params['output_i'])
